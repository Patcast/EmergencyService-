...
//GALAXY
      auto twix_galaxy = std::make_shared<MonitoredSpace>(generateId(),"Txix Galaxy");
      auto motion_sensor = std::make_shared<Motion>(generateId(),"Motion Sensor","Disney",2,1000);
...
// Planet X
      auto planet_x = std::make_shared<MonitoredSpace>(generateId(),"Planet X");
      auto sulphur_smoke_sensor = std::make_shared<Gas>(generateId(),"Sulphur Gas Sensor","Disney",1000000000);
...
//Surface Planet X Volcanos
      auto planet_x_redstone_volcano = std::make_shared<MonitoredSpace>(generateId(),"Planet X Redstone Volcano");
      auto planet_x_orangestone_volcano = std::make_shared<MonitoredSpace>(generateId(),"Planet X Orangestone Volcano");
      auto smoke_sensor_redstone= std::make_shared<Gas>(generateId(),"Redstone Smoke Sensor","Disney",1000);
      auto smoke_sensor_orangestone = std::make_shared<Gas>(generateId(),"Orangestone Smoke Sensor","Disney",1000);
...
// Planet LV
      auto planet_lv = std::make_shared<MonitoredSpace>(generateId(),"Planet LV");
      twix_galaxy->addNewComponent(planet_lv);
      auto oxygen_gas_sensor = std::make_shared<Gas>(generateId(),"Oxygen Gas Sensor","Disney",1200000000);
      ...
      auto motion_sensor_atmosphere = std::make_shared<Motion>(generateId(),"Motion Sensor Atmosphere","Disney",2,1000);
...
// Planet LV Egg Sites
      auto moonUnit = std::make_shared<MonitoredSpace>(generateId(),"Moon Unit");
      planet_lv->addNewComponent(moonUnit);
      auto smokeSensorLV = std::make_shared<Gas>(generateId(),"SmokeSensor from Moon Unit","Disney",12000);
      ...
      auto motionSensorLV = std::make_shared<Motion>(generateId(),"Motion Sensor Eggs Hatching","Disney",2,10,20,8);
      ...
      auto dweezil = std::make_shared<MonitoredSpace>(generateId(),"Dweezil");
      planet_lv->addNewComponent(dweezil);
      auto smokeSensorLVDweezil = std::make_shared<Gas>(generateId(),"SmokeSensor from Deweezil","Disney",12000);
      ...
      auto motionSensorLVDweezil = std::make_shared<Motion>(generateId(),"Motion Sensor Eggs Hatching","Disney",2,10,20,8);
